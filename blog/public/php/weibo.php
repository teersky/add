...
mysql_query('CREATE DATABASE dbs');

mysql_select_db('dbs');

$sql= <<< END
CREATE TABLE  `dbs`.`info` (
...
switch($act)
{
	case 'add':
		...
		$sql="INSERT INTO info (ID, content, time, acc, ref) VALUES(0, '{$content}', {$time}, 0, 0)";
		...
		break;
	case 'get_page_count':
		$sql="SELECT ceil(COUNT(*)/{$PAGE_SIZE}) FROM info";
	...
	case 'get':
		...
		$sql="SELECT ID, content, time, acc, ref FROM info ORDER BY time DESC LIMIT {$s}, {$PAGE_SIZE}";
	...
	case 'acc':
		...
		$res=mysql_query("SELECT acc FROM info WHERE ID={$id}");
		
		$row=mysql_fetch_array($res);
		
		$old=(int)$row[0]+1;
		
		$sql="UPDATE info SET acc={$old} WHERE ID={$id}";
	...
	case 'ref':
		...
		$res=mysql_query("SELECT ref FROM info WHERE ID={$id}");
		
		$row=mysql_fetch_array($res);
		
		$old=(int)$row[0]+1;
		
		$sql="UPDATE info SET ref={$old} WHERE ID={$id}";
